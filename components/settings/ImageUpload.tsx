...
export function ImageUpload({ currentUrl, onUpload, folder, label, aspectRatio = 'square' }: ImageUploadProps) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className={`relative border-2 border-dashed border-slate-200 rounded-lg overflow-hidden transition-colors hover:border-slate-300 ${aspectRatio === 'wide' ? 'aspect-[3/1]' : 'aspect-square w-32'}`}>
        {signedUrl && (
          <>
            <img src={signedUrl} alt={label} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none' }} />
